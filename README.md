...
- **Web search for the agent** -- point it at your SearXNG instance (`--searxng` or `HOMELABMON_SEARXNG`) and the agent can look up current package versions, error fixes, and documentation on the internet when a task needs it
...
The bundled `docker-compose.yml` runs a full-featured hub node: host networking (LAN access for ARP/mDNS scanning and to reach Ollama on other machines), the Docker socket mounted read-only for container discovery, and `/` mounted read-only at `/hostfs` for disk metrics. Configuration uses `HOMELABMON_*` environment variables -- each one maps to a CLI flag with `-` becoming `_`:

```yaml
environment:
  - HOMELABMON_LLM=http://192.168.178.250:11434      # Ollama endpoint
  - HOMELABMON_LLM_MODEL=qwen3.8:27b-mlx
  - HOMELABMON_SEARXNG=https://searxng.schnigg.ie/   # SearXNG for the agent's web_search (JSON format must be enabled)
```

Additional flags can be appended to the `command` (e.g. `--no-auth`, `--exec`). Retrieve the UI auth token with:
