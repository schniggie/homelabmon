...
- [x] Production rollout complete (2026-09-15): the `zerotier` node (192.168.178.211, user cd, `--exec`) enrolled via a single chat message on the live hub and appeared in the UI on its first heartbeat; the three field fixes above (SSH identity path, auth diagnostics, uname parsing) came out of the rollout attempts
...
- [x] FreeBSD/OPNsense support (field request: enrolling the OPNsense firewall): enroll_node now branches per OS -- Linux keeps systemd, FreeBSD gets an rc.d script in /usr/local/etc/rc.d using daemon(8) (restart-on-exit, syslog logging), enabled via sysrc and started via service; HOME is pinned to /root in the script (rc scripts start without it -- same bug class as the systemd HOME fix); root targets skip sudo entirely (stock OPNsense ships none); OPNsense caveat: major upgrades wipe /usr/local additions, so re-run enrollment after them
- [x] Live-verified on real OPNsense (FreeBSD 14.2, fw-papa): fresh FreeBSD binary deployed over SSH, executes (migrations + hostinfo + observers), rc.d script installed/enabled/started via service(8), daemon keeps it running with the pinned data dir, clean stop; firewall left pristine after the test. CA round-trip was network-blocked from the test VM (inbound firewalled) but is OS-agnostic code already verified on Linux
...
| zerotier | Linux | 192.168.178.211 (agent node, `--exec`) | deployed by the AI agent via `enroll_node` | systemd (homelabmon.service) |
